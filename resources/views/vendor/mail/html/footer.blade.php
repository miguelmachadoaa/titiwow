
<tr style="background:#fff;">
    <td style="background:#fff;">
        <table style="background: #fff; border-top: 1px solid #123275;" width="90%" class="" align="center" width=""  cellpadding="0" cellspacing="0">
            <tr>
                
			                <td class="content-cell" align="left" style="color: #123275; font-style: 0.5em;">
			                    Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod  tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure
			                </td>
			          
            </tr>
        </table>
    </td>
</tr>



<tr style="background: #0199dc;">
    <td>
        <table style="background: #0199dc; border-bottom:  1px solid #fff;" width="90%" class="" align="center" width=""  cellpadding="0" cellspacing="0">
            <tr>
                <td class="" align="center">
                    <table style="background:#0199dc;" class="footer" align="center" width="570" cellpadding="0" cellspacing="0">
			            <tr>
			                <td class="content-cell" align="left" style="color: #123275; font-style: 0.5em;">
			                    <p style="text-align: center;"><img src="{{ secure_url('assets/img/login.png') }}"></p>
			                </td>
			                <td class="content-cell" align="left">
			                    <p style="text-align: left; color: #fff;">Bogota, Colombia </p>
			                    <p style="text-align: left; color: #fff;"> Soporte@alpinago.com</p>
			                    <p style="text-align: left; color: #fff;"> +57 0 0000 00000</p>
			                </td>
			            </tr>
			        </table>
                </td>
            </tr>
        </table>
    </td>
</tr>

<tr style="background: #0199dc;">
    <td>
        <table style="background: #0199dc; 		" width="90%" class="" align="center" width=""  cellpadding="0" cellspacing="0">
            <tr>
                <td class="" align="center">
                    <br>	
                </td>
            </tr>
        </table>
    </td>
</tr>







 